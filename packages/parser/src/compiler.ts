...
type EmitterPropValue = StaticValue | EmitterRandomAST | EmitterOverLifeAST | AnimationAST;
...
function createParticleElement(
  ...
  const baseX = spawnPos?.x ?? resolveEmitterPositionValue(eProps.x, localBirthMs, seed, 1, variables) ?? 0;
  const baseY = spawnPos?.y ?? resolveEmitterPositionValue(eProps.y, localBirthMs, seed, 2, variables) ?? 0;
...
function isEmitterAnimation(value: EmitterPropValue | undefined): value is AnimationAST {
  return typeof value === "object" && value !== null && "kind" in value && (value.kind === "keyframes" || value.kind === "tween" || value.kind === "expression");
}
...
function resolveEmitterNumber(
  ...
  if (isEmitterAnimation(value)) {
    throw new Error(`@emitter '${name}' cannot be animated`);
  }
...
function resolveEmitterPositionValue(
  value: EmitterPropValue | undefined,
  localBirthMs: number,
  seed: number,
  offset: number,
  ...
  if (isEmitterAnimation(value)) {
    return sampleEmitterAnimation(value, localBirthMs, variables);
  }

  return resolveEmitterRandomValue(value, seed, offset, variables);
}

function sampleEmitterAnimation(
  animation: AnimationAST,
  localTMs: number,
  variables: Record<string, StaticValue>
): number {
  if (animation.kind === "expression") {
    const source = replaceVariableRefs(animation.source, variables);
    return compileNumericExpression(source)({ t: localTMs / 1000 });
  }

  const resolved = resolveAnimation(animation, variables) as Exclude<AnimationAST, { kind: "expression" }>;
  const points = compileAnimationPoints(resolved);

  if (points.length === 0) {
    throw new Error("Cannot evaluate emitter keyframes without points");
  }

  if (localTMs <= points[0]!.t) return points[0]!.value;
  ...
  for (let index = 1; index < points.length; index += 1) {
    ...
    if (localTMs <= right.t) {
      ...
      const eased = getCompilerEasingFunction(right.easing)(progress);
...
function getCompilerEasingFunction(name: string): (t: number) => number {
  ...
  if (normalized.startsWith("cubic-bezier(") && normalized.endsWith(")")) {
    ...
    if (values.length === 4 && values.every((value) => Number.isFinite(value))) {
      return compilerCubicBezier(values[0]!, values[1]!, values[2]!, values[3]!);
    }
  }

  if (normalized.startsWith("spring(") && normalized.endsWith(")")) {
    return compilerSpring(parseCompilerSpringOptions(normalized));
  }

  if (normalized === "ease-in") return compilerCubicBezier(0.42, 0, 1, 1);
  if (normalized === "ease-out") return compilerCubicBezier(0, 0, 0.58, 1);
  if (normalized === "ease-in-out") return compilerCubicBezier(0.42, 0, 0.58, 1);
  return (t) => Math.min(1, Math.max(0, t));
}

interface CompilerSpringOptions {
  stiffness: number;
  damping: number;
  mass: number;
  velocity: number;
...
function parseCompilerSpringOptions(source: string): CompilerSpringOptions {
  const body = source.slice("spring(".length, -1).trim();
  const options: CompilerSpringOptions = {
    stiffness: 180,
    damping: 12,
    mass: 1,
    velocity: 0
...
function compilerSpring(options: CompilerSpringOptions): (t: number) => number {
  return (t: number) => {
...
function compilerCubicBezier(x1: number, y1: number, x2: number, y2: number): (t: number) => number {
  return (t: number) => {
    const x = Math.min(1, Math.max(0, t));
    const curveT = solveCompilerCurveX(x, x1, x2);
    return Math.min(1, Math.max(0, sampleCompilerCurve(curveT, y1, y2)));
  };
}

function solveCompilerCurveX(x: number, x1: number, x2: number): number {
  let t = x;
  for (let index = 0; index < 8; index += 1) {
    const estimate = sampleCompilerCurve(t, x1, x2) - x;
    if (Math.abs(estimate) < 1e-6) return t;

    const derivative = sampleCompilerDerivative(t, x1, x2);
  ...
  t = x;
  while (lower < upper) {
    const estimate = sampleCompilerCurve(t, x1, x2);
...
function sampleCompilerCurve(t: number, p1: number, p2: number): number {
  const invT = 1 - t;
  return 3 * invT * invT * t * p1 + 3 * invT * t * t * p2 + t * t * t;
}

function sampleCompilerDerivative(t: number, p1: number, p2: number): number {
